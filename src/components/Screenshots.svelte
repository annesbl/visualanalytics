...
{#if mount || visible}
  <div class="images">
    {#each [...new Array(count).keys()] as i}
      <img
        src={`/${key.includes("meme") ? "assets/img/memes" : "assets/img/screenshots"}/pic${
          picNums[i]
        }.png`}
        alt={key}
        style={`
            ...
            color: ${getColor(i)};
            top: ${visible ? destinations[i].y : origins[i].y}px;
            left: ${visible ? destinations[i].x : origins[i].x}px;
          `}
        class:big={visible}
